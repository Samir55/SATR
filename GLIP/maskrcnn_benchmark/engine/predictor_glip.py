...
class GLIPDemo(object):
    ...
    def inference_on_image(self, original_image, original_caption, thresh=0.5):
        predictions = self.compute_prediction(original_image, original_caption)
        top_predictions = self._post_process(predictions, thresh)
        ...
        return scores, boxes, new_labels
    ...
    def overlay_boxes(self, image, predictions):
        ...
        for box, color in zip(boxes, colors):
            ...
            # print(f"Box {k}")
            # print(tuple(top_left), tuple(bottom_right), tuple(color))
            # cv2.imwrite(f'/media/abdeas0a/Ahmed/research/3dh/interactiveNotebooks/glip_output/{k}_before.jpg', image)
            image = cv2.rectangle(
                image, tuple(top_left), tuple(bottom_right), tuple(color), 2)
            # cv2.imwrite(f'/media/abdeas0a/Ahmed/research/3dh/interactiveNotebooks/glip_output/{k}_after.jpg', image)
            # k +=1
